...
const AlcoveGlow = styled.div`
  ...
  ${({ $position }) =>
    $position === 'south' && 'bottom: 8%; left: 50%; transform: translateX(-50%);'}
  ...
  useEffect(() => {
    if (puzzle.complete) {
      ...
      dispatch({
        ...
        payload: { id: 'logician', title: 'Logician', description: 'Solved the Nyāya syllogism.' },
      });
      // Progress to next trial
      const t = setTimeout(
        () => dispatch({ type: ACTIONS.SET_CURRENT_SCENE, payload: SCENES.VAISESIKA_TRIAL }),
        800
      );
    ...
    <SceneContainer>
        ...
        <ProgressiveNarrative
          ...
          blocks={[
            ...
            <div>
              ...
              <span>
                <GlossaryTerm className="highlight-mystical is-interactive" term="Pratyakṣa" />{' '}
                (Perception),
                <GlossaryTerm className="highlight-mystical is-interactive" term="Anumāṇa" />{' '}
                (Inference),
                <GlossaryTerm className="highlight-mystical is-interactive" term="Upamāna" />{' '}
                (Comparison),
                <GlossaryTerm className="highlight-mystical is-interactive" term="Śabda" />{' '}
                (Testimony).
              </span>
            </div>,
          ]}
      ...
      {showPuzzle && (
        <PuzzleWrap role="region" aria-label="Nyāya syllogism puzzle">
          <InstructionText>
            Select a card, then place it into the correct logical step.
          </InstructionText>
          ...
          <Controls>
            <PlaceBtn
              className="is-interactive"
              onClick={puzzle.submit}
              disabled={!puzzle.canSubmit}
              aria-disabled={!puzzle.canSubmit}
            >
              Submit
            </PlaceBtn>
            <PlaceBtn className="is-interactive" onClick={puzzle.reset}>
              Reset
            </PlaceBtn>
            <PlaceBtn
              className="is-interactive"
          ...
          <Message role="status" $success={puzzle.complete}>
            {puzzle.message}
          </Message>
        </PuzzleWrap>
      )}
